...
func main() {
	...
	flag.BoolVar(&options.PreSyncMode, "pre-sync-mode", false, "for RWX PV only: create the new PVC and copy the data, but do not swap the PVC yet")
	flag.BoolVar(&options.RWXOnly, "rwx-only", false, "only consider RWX PVs")
