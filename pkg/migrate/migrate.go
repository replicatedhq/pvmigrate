...
func Migrate(sourceSCName, destSCName, rsyncImage string, setDefaults, verboseCopy bool) {
	...
	matchingPVCs, originalRetentionPolicies, originalPVNames, namespaces, err := getPVCs(context.TODO(), os.Stdout, clientset, sourceSCName)
	if err != nil {
		fmt.Printf("%s\n", err.Error())
		os.Exit(1)
	}
		...
		for _, nsPvc := range nsPvcs {
			...
			if err != nil {
				if !k8serrors.IsNotFound(err) {
					fmt.Printf("failed to find existing PVC: %+v\n", err)
					os.Exit(1)
				}
				...
				if existingPVC.Spec.StorageClassName != nil && *existingPVC.Spec.StorageClassName == destSCName {
					...
					desiredSize := nsPvc.Spec.Resources.Requests.Storage().String()
					if existingSize == desiredSize {
						fmt.Printf("found existing PVC with name %s, not creating new one\n", newName)
						newPVCs[ns] = append(newPVCs[ns], *existingPVC)
						continue
					} else {
						fmt.Printf("storage class %s already exists in namespace %s but with size %s instead of %s, cannot create migration target from %s\nPlease delete this to continue\n", newName, ns, existingSize, desiredSize, nsPvc.Name)
						os.Exit(1)

					}
				} else {
					fmt.Printf("storage class %s already exists in namespace %s but with storage class %v, cannot create migration target from %s\nPlease delete this to continue\n", newName, ns, existingPVC.Spec.StorageClassName, nsPvc.Name)
					os.Exit(1)
				}
			}
			...
			if err != nil {
				fmt.Printf("failed to create new PVC %s in %s: %v\n", newName, ns, err)
				os.Exit(1)
			}
			fmt.Printf("created new PVC %s with size %v in %s\n", newName, newPVC.Spec.Resources.Requests.Storage().String(), ns)
...
// getPVCs gets all of the PVCs and associated info using the given StorageClass
// returns:
// a map of namespaces to arrays of PVCs
...
func getPVCs(ctx context.Context, w io.Writer, clientset k8sclient.Interface, sourceSCName string) (map[string][]corev1.PersistentVolumeClaim, map[string]map[string]corev1.PersistentVolumeReclaimPolicy, []string, []string, error) {
	// get PVs using the specified storage provider
	pvs, err := clientset.CoreV1().PersistentVolumes().List(ctx, metav1.ListOptions{})
	if err != nil {
		return nil, nil, nil, nil, fmt.Errorf("failed to get persistent volumes: %w", err)
	}
	...
	for _, pv := range matchingPVs {
		if pv.Spec.ClaimRef != nil {
			...
			if err != nil {
				return nil, nil, nil, nil, fmt.Errorf("failed to get PVC for PV %s in %s: %w", pv.Spec.ClaimRef.Name, pv.Spec.ClaimRef.Namespace, err)
			}
			...
			return nil, nil, nil, nil, fmt.Errorf("PV %s does not have an associated PVC - resolve this before rerunning", pv.Name)
		}
	}
	...
	err = tw.Flush()
	if err != nil {
		return nil, nil, nil, nil, fmt.Errorf("failed to print PVCs: %w", err)
	}

	return matchingPVCs, originalRetentionPolicies, originalPVNames, namespaces, nil
}
