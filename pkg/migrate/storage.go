...
// buildBindVolumePod creates a pod that mounts the provided pvc under /data directory. if node
// name is provided the returned pod will have a node selector requiring the pod to be scheduled
// in the provided node. the returned pod only echoes back a string and its sole purpose is to
// make sure the pvc has been bound to a pv.
func buildBindVolumePod(namespace, pvcName, image, node string) *corev1.Pod {
	...
	return &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      buildResourceName("pvmigrate-volume-bind", pvcName),
			Namespace: namespace,
			Labels: map[string]string{
				"pvmigrate": "volume-bind",
			},
